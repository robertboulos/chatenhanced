  ...
  return (
    <div className="relative">
      ...
      <motion.button
        ...
        disabled={disabled}
        className={`flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-all duration-200 text-sm sm:text-base ${
          disabled
      ...
      >
        <div className="flex items-center space-x-2 min-w-0">
          {activeCompanion.avatar ? (
            ...
            <User size={14} className="sm:w-4 sm:h-4 text-gray-600 dark:text-zinc-400" />
          )}
          <span className="font-medium truncate max-w-20 sm:max-w-32">
            {activeCompanion.name}
          </span>
        </div>
        <ChevronDown 
          size={14} 
          className={`sm:w-4 sm:h-4 text-gray-600 dark:text-zinc-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} 
        />
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-72 sm:w-80 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-700 rounded-lg shadow-xl z-50 overflow-hidden"
          >
            {/* Header */}
            <div className="p-2 sm:p-3 bg-gray-100 dark:bg-zinc-900/50 border-b border-gray-300 dark:border-zinc-700">
              <div className="flex items-center justify-between">
                <h3 className="text-xs sm:text-sm font-medium text-gray-900 dark:text-zinc-200">AI Companions</h3>
                <button
                ...
                >
                  <Plus size={14} className="sm:w-4 sm:h-4 text-gray-600 dark:text-zinc-400" />
                </button>
              </div>
            </div>
              ...
              {companions.map((companion) => (
                ...
                >
                  <button
                    ...
                    className="w-full p-2 sm:p-3 text-left transition-colors"
                  >
                    <div className="flex items-start space-x-2 sm:space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <h4 className="text-xs sm:text-sm font-medium text-gray-900 dark:text-zinc-200 truncate">
                            {companion.name}
                          </h4>
                          {companion.id === activeCompanion.id && (
                        ...
                        <div className="hidden sm:flex items-center space-x-2 mt-2 text-xs text-gray-500 dark:text-zinc-500">
                  ...
                  <div className="absolute top-1 sm:top-2 right-1 sm:right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="flex space-x-1">
                      ...
                      >
                        <Settings size={10} className="sm:w-3 sm:h-3 text-gray-600 dark:text-zinc-400" />
                      </button>
                      ...
                      >
                        <Copy size={10} className="sm:w-3 sm:h-3 text-gray-600 dark:text-zinc-400" />
                      </button>
                      ...
                      {companions.length > 1 && (
                        ...
                        >
                          <Trash2 size={10} className="sm:w-3 sm:h-3 text-red-400" />
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-2 bg-gray-100 dark:bg-zinc-900/30 border-t border-gray-300 dark:border-zinc-700">
              <button
                className="w-full p-1.5 sm:p-2 text-xs sm:text-sm text-gray-600 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-200 hover:bg-gray-200 dark:hover:bg-zinc-700 rounded transition-colors flex items-center justify-center space-x-2"
              >
